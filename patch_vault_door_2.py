...
if target in content:
    ...
    print("Exact block NOT found! Let's do regex.")
    # More robust regex replacement
    pattern = r"    Box\(modifier = modifier\.size\(96\.dp\), contentAlignment = Alignment\.Center\) \{.*?\n    \}"
    if re.search(pattern, content, flags=re.DOTALL):
        print("Regex found it!")
        content = re.sub(pattern, replacement, content, flags=re.DOTALL)
    else:
        print("Regex didn't find it either.")
