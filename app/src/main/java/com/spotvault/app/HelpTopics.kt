...
val HelpTopics: List<Pair<String, List<HelpItem>>> = listOf(
    ...
    "Quick Pins" to listOf(
        ...
        HelpItem(Icons.Default.PushPin, "Auto-Close After Save", "Off keeps the Quick-Pin sheet open after a save instead of dismissing it, so you can drop several pins in a row without reopening it each time. Set in Quick-Pin Behavior settings.", settingsCategoryId = "quickpin"),
        HelpItem(Icons.Default.FlashOn, "Home Screen Shortcuts", "Show or hide the Quick Pin / Quick Track row below Snap & Pin on the home screen. Set in Quick-Pin Behavior settings.", settingsCategoryId = "quickpin")
    ),
    "The Vault" to listOf(
        ...
        HelpItem(Icons.Default.Star, "Favorites", "Swipe or tap the star on any spot to favorite it — favorites are protected from Auto Delete. Tap the Star icon at the top right of the Vault (colored to match your current theme) to open your dedicated Favorites Hub: every favorite lives there, ready to pull up any time you need to quickly find a spot you visit routinely, plus a + Add Spot button to save somewhere you want to go — type an address, search for it in Maps, or share a place into the app (see below) — even if you've never actually been there to drop a pin. The form itself has its own + Add Photo button (asks Camera or Gallery) and voice-to-text on every field."),
        ...
        HelpItem(Icons.Default.Inventory2, "Default Save Mode", "Choose whether a new Snap or Pin starts as Active Tracking (a live timer with a persistent notification) or a Quiet Save (straight to the Vault, no timer) by default. Set in Vault settings under Save Defaults.", settingsCategoryId = "vault"),
        HelpItem(Icons.Default.Inventory2, "Default Timer (minutes)", "How long an active-tracking timer runs by default when you don't pick a length yourself. Set in Vault settings under Save Defaults.", settingsCategoryId = "vault"),
        HelpItem(Icons.Default.Inventory2, "Default Sort Order", "Choose how the Vault list sorts your spots by default — newest first, oldest first, or others. Set in Vault settings under Vault Behavior.", settingsCategoryId = "vault")
    ),
    "Vehicles & Automatic Parking" to listOf(
        ...
        HelpItem(Icons.Default.BatteryAlert, "Battery Optimization", "Some phones (Samsung, Xiaomi, and others) kill background Bluetooth detection and the active-tracking notification unless DropPin Vault is exempted from battery optimization — this is the single biggest reliability fix for both Automatic Parking and active tracking. Exempt it right from the checklist in Automatic Parking settings — the same exemption covers active tracking too.", settingsCategoryId = "autopark")
    ),
    ...
    "Sounds & Alerts" to listOf(
        ...
        HelpItem(Icons.Default.LocationOn, "Keep Screen On While Tracking", "Prevents your phone from sleeping while an active-tracking timer is running. Set in Sounds & Alerts settings under Timers & Notifications.", settingsCategoryId = "timer")
    ),
    ...
    "Widgets" to listOf(
        ...
        HelpItem(Icons.Default.Palette, "Widget theming", "Widgets automatically match your in-app color theme when \"Sync Widget Colors With Theme\" is on, and your chosen button shape too when \"Sync Widget Layout With Theme\" is also on — turn the layout one off to keep widget buttons on the default Rounded shape regardless of your in-app Button Style. Both in Widget settings.", settingsCategoryId = "widgets"),
        HelpItem(Icons.Default.Widgets, "Refresh Widgets on App Open", "Forces widgets to update the instant you open the app, instead of waiting for Android's normal ~30-minute background widget-update limit. On by default. Set in Widget settings.", settingsCategoryId = "widgets"),
